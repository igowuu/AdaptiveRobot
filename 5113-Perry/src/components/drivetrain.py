...
class Drivetrain(AdaptiveComponent):
    ...
    def __init__(self, robot: "AdaptiveRobot") -> None:
        ...
        self._forward_axis_controller = AxisController(0.0, DrivetrainConst.SLEW_FORWARD)
        self._rotation_axis_controller = AxisController(0.0, DrivetrainConst.SLEW_ROTATION)
    ...
    def stop(self) -> None:
        ...
        self._forward_axis_controller.request(
            AxisRequest(0.0, DrivePriority.SAFETY.value, 0.2, "safety")
        )
        self._rotation_axis_controller.request(
            AxisRequest(0.0, DrivePriority.SAFETY.value, 0.2, "safety")
        )
    ...
    def request_forward_percent(
        ...
        """
        forward = applyDeadband(forward_speed, JoystickConst.DEADBAND)
    ...
    def request_rotation_percent(
        ...
        rotation = clamp(rotation, -self.max_pct_output.value, self.max_pct_output.value)
    ...
    def publish_telemetry(self) -> None:
        ...
        self.publish_value(
            "Drive/FLVelocity (rotations per minute)",
            self.front_left_encoder.getVelocity()
        )
        self.publish_value(
            "Drive/FRVelocity (rotations per minute)",
        ...
        resolved_vx_request = self._forward_axis_controller.last_output
        ...
        if self._forward_requests:
            best_forward = resolve_axis(self._forward_requests)
        ...
        self.publish_value("Drive/DriveAxis/ResolvedVX (m per s)", resolved_vx_request)
        self.publish_value("Drive/DriveAxis/ResolvedOmega (rad per s)", resolved_omega_request)
    ...
    def execute(self) -> None:
        ...
        vx = self._forward_axis_controller.resolve()
        omega = self._rotation_axis_controller.resolve()
        ...
        self._forward_axis_controller.clear()
        self._rotation_axis_controller.clear()
