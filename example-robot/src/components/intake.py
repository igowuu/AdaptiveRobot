...
from config.constants import JoystickConst, IntakeConst
...
class Intake(AdaptiveComponent):
    ...
    def stop(self) -> None:
        self._move_requests.append(
            AxisRequest(0.0, IntakePriority.SAFETY.value, "safety")
        )
    ...
    def move_intake(self, desired_pct_output: percent) -> None:
        ...
        desired_pct_output = clamp(
            desired_pct_output, 
            -IntakeConst.MAX_PCT_OUTPUT, 
            IntakeConst.MAX_PCT_OUTPUT
        )

        self._move_requests.append(
            AxisRequest(desired_pct_output, IntakePriority.TELEOP.value, "teleop")
        )
